...
export default function PartDetailPage() {
  ...
  if (!part) return (
    <div className="flex flex-col items-center justify-center h-64 gap-3">
      ...
      <Link href="/parts" className="text-xs text-[#5c5fef] hover:underline">Back to parts</Link>
    </div>
  )
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        {[
          ...
          { label: 'Unit Margin', value: `${margin}%`, color: 'text-[#5c5fef]' },
          { label: 'Stock Value', value: formatCurrency(inventoryValue), color: 'text-[#111827]' },
        ].map(stat => (
          <div key={stat.label} className="bg-white border border-[#e5e7eb] rounded-lg p-3 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
                ...
                return (
                  <tr key={u.id} className="h-10 hover:bg-[#f9fafb] transition-colors">
                    <td className="px-3 py-2.5">
                      <Link href={`/service/${u.service_call_id}`} className="font-mono text-xs text-[#5c5fef] hover:underline">
