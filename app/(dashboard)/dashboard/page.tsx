...
function formatPeriod(start: string | null, end: string | null): string {
  if (!start || !end) return '—'
  return `${PERIOD_DATE_FORMATTER.format(new Date(`${start}T00:00:00`))} – ${formatDate(end)}`
}
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
        <StatCard
          ...
          value={mrrFormatted}
          sub={`from ${m.active_customers} customers · ${m.active_contracts} contracts`}
          color="info"
          icon={TrendingUp}
      ...
      {/* Meter progress bar */}
      <div className="bg-white border border-[#ebebeb] rounded-xl p-5 mb-4 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
          <div className="flex items-center gap-3 min-w-[240px]">
            <div className="w-10 h-10 rounded-full bg-[#eff6ff] text-[#2563eb] border border-[#bfdbfe] flex items-center justify-center flex-shrink-0">
              <Gauge className="w-5 h-5" />
            </div>
            <div>
              <div className="text-xs text-[#111827] font-semibold mb-1">Meter Collection Progress</div>
              <div className="text-2xl font-semibold text-[#111827] leading-none">
                <span className="tabular-nums">{m.meters_collected}</span>
                <span className="text-[#6b7280] text-base font-normal"> of </span>
                <span className="tabular-nums">{m.meters_due}</span>
                <span className="text-sm font-medium text-[#374151] ml-1.5">meters collected</span>
              </div>
            </div>
          </div>
          <div className="h-2 bg-[#e5e7eb] rounded-full overflow-hidden flex-1 min-w-[180px]">
            <div
              className="h-full bg-[#2563eb] rounded-full transition-all duration-700"
              style={{ width: `${meterPct}%` }}
            />
          </div>
          <span className="text-xl font-semibold tabular-nums text-[#111827] lg:w-14 lg:text-right">{meterPct}%</span>
          <Link href="/meters" className="lg:ml-4">
        ...
        <div className="bg-white border border-[#ebebeb] rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          ...
          <div className="divide-y divide-[#f7f7f7]">
            {openCalls.slice(0, 5).map(call => {
              ...
              return (
                <Link key={call.id} href={`/service/${call.id}`} className="flex items-center gap-3 px-4 py-2 hover:bg-[#fafafa] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[#6b7280] truncate">{call.problem_description?.slice(0, 58)}…</div>
                  </div>
        ...
        <div className="bg-white border border-[#ebebeb] rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
          ...
          <div className="divide-y divide-[#f7f7f7]">
            {expiringContracts.slice(0, 5).map(contract => {
              ...
              return (
                <Link key={contract.id} href={`/contracts/${contract.id}`} className="flex items-center gap-3 px-4 py-2 hover:bg-[#fafafa] transition-colors">
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="text-right flex-shrink-0">
                    <div className={`text-xs font-semibold tabular-nums ${isExpired ? 'text-[#dc2626]' : days <= 30 ? 'text-[#dc2626]' : 'text-[#d97706]'}`}>
                      {isExpired ? `${Math.abs(days)} days ago` : `${days} days left`}
                    </div>
                    <div className="text-xs text-[#9ca3af] mt-1">{formatDate(contract.end_date)}</div>
                  </div>
                </Link>
              )
            })}
          ...
          <Tbody>
            {recentInvoices.map(inv => (
              <Tr key={inv.id}>
                ...
                <Td><span className="text-xs text-[#6b7280]">{formatPeriod(inv.billing_period_start, inv.billing_period_end)}</span></Td>
